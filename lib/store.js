...
function makeTask(title, source = "manual", extra = {}) {
  ...
  return {
    ...
    pri: TASK_PRIS.includes(extra.pri) ? extra.pri : "",      // 任意の優先度（高/中/低）。未指定は""
...
export async function updateTask(date, id, fields = {}) {
  ...
  if (typeof fields.pri === "string") task.pri = TASK_PRIS.includes(fields.pri) ? fields.pri : "";
...
export async function setSchedule(date, items = [], source = "ai") {
  ...
  day.tasks = items.map((it) => {
    ...
    const t = makeTask(title, source, { time: it.time, endTime: it.endTime, cat: it.cat, tags: it.tags, memo: it.memo, pri: it.pri });
...
// --- iCloudカレンダー連携の認証情報 ---
// パスワード(アプリ用パスワード)を含むため、汎用ドキュメント(doc:*)とは別の
// キー名前空間(icloudAuth)に保存する。/api/data/:key は "doc:"+key しか読めないので、
// この関数を経由しない限りクライアントに漏れることはない。
export async function getIcloudAuth() {
  await init();
  const rs = await db.execute({ sql: "SELECT value FROM kv WHERE key = ?", args: ["icloudAuth"] });
  return rs.rows.length ? JSON.parse(rs.rows[0].value) : null;
}
export async function setIcloudAuth(auth) {
  await init();
  await db.execute({
    sql: "INSERT INTO kv (key, value) VALUES ('icloudAuth', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value",
    args: [JSON.stringify(auth ?? null)],
  });
}
export async function clearIcloudAuth() {
  await init();
  await db.execute({ sql: "DELETE FROM kv WHERE key = 'icloudAuth'", args: [] });
}
...
// 収入・支出の合計。month省略で全期間。確定分（pending=未収・引き落とし待ちを除く）のみを、
// 発生日(date)基準で合計する。残高・確定合計・今月の収支はこれで揃える（=銀行に反映済みの実額）。
// pendingは含めない代わりに、月をまたいだ引き落としでも「使った月」に正しく計上される（決済日ズレのバグは解消したまま）。
...
// 現在pending中（未収の収入/引き落とし待ちの支出）の合計。残高・確定合計には含まれないため、
// 「予想収支」側（projectedBalance）でのみ加算して将来の着地見込みを示す。
async function sumPendingTx() {
  await init();
  await autoSettlePending();
  const rs = await db.execute(
    "SELECT type, COALESCE(SUM(amount), 0) AS total FROM transactions WHERE payoutStatus = 'pending' GROUP BY type"
  );
  const out = { income: 0, expense: 0 };
  for (const r of rs.rows) out[r.type] = Number(r.total);
  return out;
...
export async function financeSummary(month = today().slice(0, 7)) {
  await ensureMigrated();
  const [{ initialBalance }, all, m, pending] = await Promise.all([getFinance(), sumTx(), sumTx(month), sumPendingTx()]);
  // currentBalance（確定残高）は確定分のみ＝今の口座の実額。
  // 全期間の収入合計/支出合計は「未収・引き落とし予定も含めた総額」を出す（＝将来の着地見込みの内訳）。
  const currentBalance = initialBalance + all.income - all.expense;
  return {
    initialBalance,
    currentBalance,                         // 確定残高（確定分のみ）
    month,
    incomeThisMonth: m.income,              // 今月の収入（確定分・発生日ベース）
    expenseThisMonth: m.expense,            // 今月の支出（確定分・発生日ベース）
    netThisMonth: m.income - m.expense,
    totalIncome: all.income + pending.income,     // 全期間の収入合計（未収を含む）
    totalExpense: all.expense + pending.expense,  // 全期間の支出合計（引き落とし予定を含む）
    confirmedIncome: all.income,            // 参考: 確定分のみの全期間収入
    confirmedExpense: all.expense,          // 参考: 確定分のみの全期間支出
    pendingIncome: pending.income,
    pendingExpense: pending.expense,
    projectedBalance: currentBalance + pending.income - pending.expense, // 最終予想残高＝初期残高+totalIncome-totalExpense
  };
}
