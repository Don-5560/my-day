...
// 収入・支出の合計。month省略で全期間。発生日(date)基準で、pending（未収・引き落とし待ち）分も含めて
// 「実際に使った/稼いだ金額」をそのまま合計する（残高・確定合計・今月の収支はこれで揃える）。
...
// 現在pending中（未収の収入/引き落とし待ちの支出）の合計。sumTxの合計に既に含まれているため、
// 参考値（銀行にまだ反映されていない内訳）として返すのみで、残高には加算し直さない。
