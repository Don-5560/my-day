...
async function init() {
  ...
  ready = (async () => {
    ...
    await ensureColumn("transactions", "payoutStatus", "TEXT"); // "pending" | "settled" | null(勤務先と無関係な取引)
...
// 給料日を過ぎた pending 取引を settled にする（自動入金確定）。
// このタイミングで初めて売上明細(doc:sales)にもミラーする。
async function autoSettlePending() {
  await init();
  const due = await db.execute({
    sql: "SELECT id, date, category, amount, memo FROM transactions WHERE payoutStatus = 'pending' AND payoutDate <= ?",
    args: [today()],
  });
...
// カテゴリー→勤務先の対応は「カテゴリーに employerId が直接ある」か「勤務先の linkedCategory がカテゴリー名と一致」のどちらか。
// payoutStatus は変更しない（null=すでに残高に反映済み）ので、遡って未収化はしない。表示（カレンダー/一覧の給料日まとめ）だけ後から効くようにする。
async function backfillEmployerLinks() {
  await init();
  const rs = await db.execute("SELECT id, date, category FROM transactions WHERE type = 'income' AND employerId IS NULL");
  ...
  for (const row of rs.rows) {
    ...
    await db.execute({ sql: "UPDATE transactions SET employerId = ?, payoutDate = ? WHERE id = ?", args: [emp.id, payoutDate, row.id] });
  }
}
...
// employerId を渡すと、その勤務先の給与サイクルに従って payoutStatus="pending" にし、
// 給料日(payoutDate)を過ぎるまで残高・売上明細に反映しない（autoSettlePendingが確定させる）。
export async function addTransaction({ type, amount, category, date, memo, source, employerId, hours } = {}) {
  ...
  await init();
  await db.execute({
    sql: `INSERT INTO transactions (id, date, type, category, amount, memo, source, createdAt, employerId, payoutStatus, payoutDate, hours)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [row.id, row.date, row.type, row.category, row.amount, row.memo, row.source, row.createdAt, row.employerId, row.payoutStatus, row.payoutDate, row.hours],
  });
...
export async function listTransactions({ month, type } = {}) {
  ...
  const rs = await db.execute({
    sql: "SELECT id, date, type, category, amount, memo, source, employerId, payoutStatus, payoutDate, hours FROM transactions" +
...
// 収入・支出の合計。month省略で全期間。pending（未収）の収入は残高・集計に含めない。
async function sumTx(month) {
  await init();
  await autoSettlePending();
  const rs = await db.execute({
    sql: `SELECT type, COALESCE(SUM(amount), 0) AS total FROM transactions
      WHERE (type = 'expense' OR payoutStatus IS NULL OR payoutStatus != 'pending')
