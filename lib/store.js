  ...
  if (rs.rows.length) {
    ...
    for (const row of rs.rows) {
      if (row.type === "income") {
        ...
        const payoutStatus = chargeDate > t ? "pending" : null;
        await db.execute({ sql: "UPDATE transactions SET creditCardId = ?, payoutDate = ?, payoutStatus = ? WHERE id = ?", args: [card.id, chargeDate, payoutStatus, row.id] });
      }
    }
  }
  // 補修: 上のロジックの旧版（payoutStatusを一切変更しなかった版）で、支払い日が未来なのに
  // pending化され損ねたまま残ってしまった行を直す（1回限りの補修。該当なければ何もしない）
  await db.execute({
