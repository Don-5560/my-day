...
// 既存の（勤務先/カードに紐づく前に記録された）取引に、あとから employerId/creditCardId・支払い日を遡って補完する。
// カテゴリー→勤務先/カードの対応は「カテゴリーに直接IDがある」か「勤務先側のlinkedCategoryがカテゴリー名と一致」のどちらか。
// payoutStatus は変更しない（null=すでに残高に反映済み）ので、遡ってpending化はしない。表示（カレンダー/一覧のまとめ）だけ後から効くようにする。
async function backfillLinkedTx() {
  await init();
  const rs = await db.execute(
    "SELECT id, date, type, category FROM transactions WHERE (type = 'income' AND employerId IS NULL) OR (type = 'expense' AND creditCardId IS NULL)"
  );
  ...
  const findCreditCard = (catName) => {
    const cat = (cdoc.expense || []).find((c) => (typeof c === "string" ? c : c?.name) === catName);
    if (cat && typeof cat === "object" && cat.creditCardId) {
      const c = ccdoc.items.find((x) => x.id === cat.creditCardId);
      if (c) return c;
    }
    return ccdoc.items.find((c) => c.linkedCategory === catName) || null;
  };
  for (const row of rs.rows) {
    if (row.type === "income") {
      ...
      const card = findCreditCard(row.category);
      if (!card) continue;
      const chargeDate = computeChargeDate(card, row.date);
      await db.execute({ sql: "UPDATE transactions SET creditCardId = ?, payoutDate = ? WHERE id = ?", args: [card.id, chargeDate, row.id] });
    }
  }
}
