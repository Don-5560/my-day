...
export async function recordSale({ amount, cost = 0, source, date, memo } = {}) {
  ...
  const d = date || today();
  const inc = await addTransaction({ type: "income", amount: amt, category: source || "その他", date: d, memo, source: "sale" });
  let expTx = null;
  if (cst > 0) {
