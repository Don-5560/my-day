...
function makeTask(title, source = "manual", time = "") {
  return {
    id: randomUUID(),
    title: String(title).trim(),
    done: false,
    time: typeof time === "string" ? time : "", // 任意の時刻 "HH:MM"（予定表示用）
...
export async function addTask(date, title, source = "manual", time = "") {
  if (!title || !String(title).trim()) throw new Error("タスク名が空です");
  const day = await getDay(date);
  const task = makeTask(title, source, time);
  day.tasks.push(task);
  await saveDay(day);
  return { day, task };
}
